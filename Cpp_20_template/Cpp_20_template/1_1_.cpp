...
// 模板 的两大好处
// 1. 控制容器中存放的数据的类型
// 2. 模板控制代码的逻辑: 适配器模式：传入不同的容器或迭代器类型，可以适配出不同的容器和迭代器
...
void Print(const Container& con) {
	// 不加 typename 时，编译前要先进行词法分析，此时Container尚未实例化，
	// 编译器不知道 Container::const_iterator 表示静态成员变量，还是类中 typedef 的类型，不能确定
	// 实例化之后才能确定类型
	...
	// typename  告诉编译器 这里是类型，等模板实例化后，再去类内找 类型
	// 只要是 取 未实例化的模板内的内容，都不能确定 取的是 类型 还是 成员变量
	//auto it = con.begin();	可以用 auto auto的推导结果，表示的一定是类型
	while (it != con.end()) {
		cout << *it << " ";
		++it;
	}
